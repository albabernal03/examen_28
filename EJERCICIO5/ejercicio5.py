...
#cada carácter deberá ser encriptado a ocho caracteres; esto significa 

def encriptar(cadena):
    cadena_encriptada = ''
    for caracter in cadena:
        cadena_encriptada += str(ord(caracter)).zfill(3) 
...
def desencriptar(cadena):
    ...
    for i in range(0, len(cadena), 3):
        cadena_desencriptada += diccionario_ascii_inverido[cadena[i:i+3]]
        with open('desencriptado.txt', 'w') as archivo:
            archivo.write(cadena_desencriptada)
